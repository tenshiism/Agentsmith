...
MODES = {
    "gentle": {
        ...
        "min_commentary_interval": 12.0,
        "shared_ai_cooldown": True,
...
class AgentBrain:
    ...
    def set_config(self, cfg: dict):
        ...
        if action_provider and action_model:
            self.llm.set_model(action_provider, action_model, cfg.get("action_model_base_url", ""))
            changed = True

        commentary_provider = cfg.get("commentary_model_provider")
        commentary_model = cfg.get("commentary_model_name")
        if commentary_provider and commentary_model:
            self.llm_commentary.set_model(commentary_provider, commentary_model, cfg.get("commentary_model_base_url", ""))
            changed = True
    ...
    async def run(self, headless: bool = False):
        ...
        if self.status == "running":
            ...
            try:
                commentary_text = await self.llm_commentary.commentate(self._build_commentary_prompt(state))
        ...
        while not state.done:
            ...
            if self.status == "running" and self._llm_cooldown_ok() and not self._tts_busy():
                print(f"[Agent] Frame {self.frame_count} — deciding action...")
                try:
                    chosen_action = await self._run_ai_call(self._decide_action(state, actions))
            ...
            if self.status == "running" and self._commentary_cooldown_ok() and not self._tts_busy():
                print(f"[Agent] Generating commentary (frame {self.frame_count})...")
                state = self.game.capture_state()
                commentary_prompt = self._build_commentary_prompt(state)
                try:
                    commentary_text = await self._run_ai_call(self.llm_commentary.commentate(commentary_prompt))
                    self.commentary.speak(commentary_text)
                    self.last_commentary_time = time.time()
                    print(f"[Commentary] {commentary_text}")
                except (asyncio.CancelledError, RuntimeError):
            ...
            if self.overlay and self.frame_count % 3 == 0:
    ...
    def _build_commentary_prompt(self, state) -> list:
        ...
        return [
            ...
            {"role": "user", "content": f"You are playing {game_name} on a 4x4 grid. The screenshot shows the current tile positions. Merge matching tiles to reach 2048. [Elapsed: {time.time() - self.start_time:.0f}s]\nRAM: {ram_summary}\nCommentate on what you see."},
        ]

    async def _decide_action(self, state, actions: list[list[int]]) -> list[int]:
        action_names = self.game.get_action_names()
        result = "none"

        prompt = self._build_prompt(state, with_image=True)
        try:
            result = await self.llm.choose_action(prompt, action_names)
        except Exception as e:
            print(f"[Agent] Primary action LLM failed: {e}")
            if self.llm_fallback:
                print("[Agent] Trying fallback LLM...")
                try:
                    result = await self.llm_fallback.choose_action(prompt, action_names)
